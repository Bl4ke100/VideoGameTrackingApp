...
public class Playing extends javax.swing.JPanel {
    ...
    public void loadPlayingGames() {
        try {
            ResultSet resultSet = Database.executeSearch("SELECT * FROM `mygame` JOIN `user` ON `user`.`username` = `mygame`.`user_username` JOIN `type` ON `mygame`.`type_id` = `type`.`id` WHERE `user`.`username` = '" + username + "' AND `type`.`id` = '2'");
    ...
    private void initComponents() {
        ...
        jButton3.setText("Remove from Playing");
...
int row = jTable1.getSelectedRow();
        ...
        try {
            Database.executeIUD("DELETE FROM `mygame` WHERE `name` = '" + name + "' AND `user_username` = '" + username + "' AND `type_id` = '2' ");
            JOptionPane.showMessageDialog(this, "Game removed from Playing", "Success", JOptionPane.INFORMATION_MESSAGE);
        } catch (Exception e) {
            e.printStackTrace();
        }
        loadPlayingGames();
