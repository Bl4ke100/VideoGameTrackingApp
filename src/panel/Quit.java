 ...
 * @author janin
 */
public class Quit extends javax.swing.JPanel {
    ...
    public void loadQuitGames() {
        try {
            ResultSet resultSet = Database.executeSearch("SELECT * FROM `mygame` JOIN `user` ON `user`.`username` = `mygame`.`user_username` JOIN `type` ON `mygame`.`type_id` = `type`.`id` WHERE `user`.`username` = '" + username + "' AND `type`.`id` = '4'");
    ...
    private void initComponents() {
        ...
        jButton9.setText("Remove from Quit");
    ...
    private void jButton9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton9ActionPerformed
        ...
        try {
            Database.executeIUD("DELETE FROM `mygame` WHERE `name` = '" + name + "' AND `user_username` = '" + username + "' AND `type_id` = '4' ");
            JOptionPane.showMessageDialog(this, "Game removed from Quit", "Success", JOptionPane.INFORMATION_MESSAGE);
        } catch (Exception e) {
            e.printStackTrace();
        }
        loadQuitGames();
